#!/usr/bin/env perl
# $Id: eval08.pl,v 1.7 2008/06/03 11:30:00 mihais Exp $
...
my ($usage) = <<EOT
  ...
  Optional parameters:
     ...
     -x : evalb:       produce output in a format similar to evalb SX
                       (http://nlp.cs.nyu.edu/evalb/); use together with -q
    -y : evalb:       produce output in a format similar to evalb SS
                       (http://nlp.cs.nyu.edu/evalb/); use together with -q
    -z FILE : output: print evalb to FILE (default is standard output)
...
our ($opt_g, $opt_s, $opt_o, $opt_h, $opt_v, $opt_q, $opt_p, $opt_u, $opt_b, $opt_x, $opt_y, $opt_z, $opt_d) ;
...
{
    my ($gold_sent, $sys_sent, $counts, $ra_tpry) = @_;
    ...
    $ra_tpry->[0] = $sent_counts{corl_prop} + $sent_counts{corl_arg}; # |G ^ M|
    $ra_tpry->[1] = $sent_counts{tot_prop} + $sent_counts{tot_arg}, # |G|
    $ra_tpry->[2] = $sent_counts{pred_prop} + $sent_counts{pred_arg}; # |M|
    # print STDERR "! $ra_tpry->[0] $ra_tpry->[1] $ra_tpry->[2]\n";
...
getopts("g:o:s:qvhpubdxyz") ;

if (defined $opt_v)
{
    my $id = '$Id: eval08.pl,v 1.7 2008/06/03 11:30:00 mihais Exp $';
    my @parts = split ' ',$id;
...
if (defined $opt_x) {  # produce output similar to evalb
    print OUT "# SX EVALB  ==================================================================\n";
}

if (defined $opt_y) {  # produce output similar to evalb
    print OUT "# SS EVALB  ==================================================================\n";
}
...
{ # main reading loop
  ...
  my @tpry = ();
  update_srl_scores($gold_srl_sent, $sys_srl_sent, \%srl_counts, \@tpry);
  # print STDERR "!!! $tpry[0] $tpry[1] $tpry[2]\n";
  ...
  if (defined $opt_x) { # produce output similar to evalb SYNTAX
      if ($word_num > 0) {
	  my ($unlabeled,$labeled) = ('NaN', 'NaN');
	  if ($sent_counts{tot} > 0) { # there are scoring tokens
	      $labeled   = 100-$sent_counts{err_any} *100.0/$sent_counts{tot};
	  ...
	  $sent_counts{tot}-$sent_counts{err_any}, 
	  $sent_counts{tot};
	  $sumt += $sent_counts{tot}-$sent_counts{err_any};
	  $sumr += $sent_counts{tot};
	  $sump += $sent_counts{tot};
      }
  }
  if (defined $opt_y) { # produce output similar to evalb SEMANTICS
      if ($word_num > 0) {
	  my ($recall,$precision) = ('NaN', 'NaN');
	  if ($tpry[1] > 0) { # there are scoring tokens
	      $recall = $tpry[0] *100.0/$tpry[1];
	  }
	  if ($tpry[2] > 0) { # there are scoring tokens
	      $precision   = $tpry[0] *100.0/$tpry[2];
	  }
	  printf OUT "  %4d %4d    0  %6.2f %6.2f  %4d    %4d        %4d    0 0 0 0\n", 
	  $sent_num, $word_num, 
	  $recall, $precision, 
