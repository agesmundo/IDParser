#!/usr/bin/env perl
# $Id: eval08.pl,v 1.6 2008/05/30 17:00:00 mihais Exp $
...
if (defined $opt_v)
{
    my $id = '$Id: eval08.pl,v 1.6 2008/05/30 17:00:00 mihais Exp $';
    my @parts = split ' ',$id;
...
my ($row_incorrect, $cell_incorrect_tot, $cell_correct_pos_tot, $cell_correct_neg_tot);
my ($global_row_incorrect,$global_cell_incorrect,$global_cell_correct_pos,$global_cell_correct_neg,$global_word_count);
...
#  update_matrix($gold_srl_sent, $sys_srl_sent, \@sent_gold, \@sent_sys);
 ($row_incorrect, $cell_incorrect_tot, $cell_correct_pos_tot, $cell_correct_neg_tot) = update_matrix($gold_srl_sent,$sys_srl_sent,\@sent_gold,\@sent_sys);


$global_row_incorrect += $row_incorrect;  # print  " global row incorrect: $global_row_incorrect\n";
#my $global_row_correct   =+ $row_correct_pos; print " global row correct: $global_row_correct\n";

$global_cell_incorrect   += $cell_incorrect_tot;   #        print  " global cell incorrect: $global_cell_incorrect\n";
$global_cell_correct_pos += $cell_correct_pos_tot;  #  print  " global cell correct pos: $global_cell_correct_pos\n";
$global_cell_correct_neg += $cell_correct_neg_tot;  #  print  " global cell correct neg: $global_cell_correct_neg\n";

$word_num = scalar @sent_gold ;
$global_word_count += $word_num ;
#PGM 19.09.08 end


# to be uncommented if PGM portion in commented
# $word_num = scalar @sent_gold ;
...
# the new measure
#
###update of global corpus-wise counts
printf OUT "  NEW MEASURE:\n";

my $rowwise_accuracy = 100-($global_row_incorrect*100.0 /$global_word_count);
printf OUT "  Row-wise_accuracy:          100- (%d * 100 / %d) = %.2f %%\n",
               $global_row_incorrect, $global_word_count, $rowwise_accuracy;


my $cellwise_accuracy = $global_cell_correct_pos*100.0 /($global_cell_incorrect + $global_cell_correct_pos);
printf OUT  "  Cell-wise_accuracy:         %d * 100 / %d + %d  = %.2f %% \n \n",
               $global_cell_correct_pos, $global_cell_incorrect, $global_cell_correct_pos, $cellwise_accuracy;


#my $std_accuracy = ($global_cell_correct_pos + $global_cell_correct_neg - $word_num)*100.0/($global_cell_incorrect + $global_cell_correct_pos + $global_cell_correct_neg - $word_num);
#printf "Standard accuracy: %.2f %%  ",$std_accuracy;
#print "\n\n";
#
#
#
...
printf OUT "  Exact match:                %d / %d * 100 = %.2f %%\n", 
...
sub update_matrix{
 ...
 my ($gold_srl_sent,$sys_srl_sent,$sent_gold,$sent_sys) = @_ ;
#  my ($line_g, $line_s, $new_sent) ;
 # my (%fields_g, %fields_s) ;
 # my ($gold_lines, $sys_lines); # all the lines for the current sentence
 # my ($gold_srl_sent, $sys_srl_sent); # the SRL::sentence objects for this sentence
...
#  $new_sent = 1 ;
#print "######gold srl sents#######################\n";
#print Dumper($gold_srl_sent);
# print "######sys srl sent#######################\n";
#print Dumper($sys_srl_sent);
#print "#######sent golds######################\n";
#print Dumper(@sent_gold);
#print "#######sent sys######################\n";
#print Dumper(@sent_sys);
#print Dumper(@sent_sys);
#print "#######end sent######################\n\n";
  my (%matrix_g,%matrix_s);
 
my $word_num = scalar @sent_gold;

# initialise
  foreach my $i (0 .. $word_num)  {
     foreach my $j (0 .. $word_num)  {
...
foreach my $i_w (0 .. $word_num-1)
  { # loop on words

    ($word, $pos, $head_g, $dep_g)
      = @{$sent_gold[$i_w]}{'word', 'pos', 'head', 'dep'} ;
    push (@{$matrix_g{$i_w+1}{$head_g}},$dep_g);

    ($word, $pos, $head_s, $dep_s)
      = @{$sent_sys[$i_w]}{'word', 'pos', 'head', 'dep'} ;
    push (@{$matrix_s{$i_w+1}{$head_s}},$dep_s);

  }



 foreach my $gold_prop ($gold_srl_sent->props){
     my @args = $gold_prop->args; #foreach my $a (@args){ print $a->position; print $gold_prop->position; print $a->label}; print "\n";

     foreach my $arg (@args){
	 push(@{$matrix_g{$arg->position+1}{$gold_prop->position+1}},$arg->label);
     }
 }

foreach my $sys_prop ($sys_srl_sent->props){
     my @args = $sys_prop->args;#foreach my $a (@args){ print $a->position; print $a->label};print "\n";

     foreach my $arg (@args){
	 push(@{$matrix_s{$arg->position+1}{$sys_prop->position+1}},$arg->label);
     }
 }
#print Dumper(%matrix_g);
#print Dumper(%matrix_s);
...
#updating at the end of every sentence and updating in total
# sentence score update by traversing the matrix

#print the whole thing gold
#  foreach my $row ( sort {$a <=>  $b} keys %matrix_g ) {
#      print "$row: { ";
#      for my $col (sort {$a <=>  $b} keys %{$matrix_g{$row} } ) {
#          print "$col  ";
#          my @a=@{$matrix_g{$row}{$col}};
#          foreach my $a (@a){ print $a};
#      }
#      print "}\n";
#  }

# print the whole thing sys
#  foreach my $row ( sort {$a <=>  $b} keys %matrix_s ) {
#      print "$row: { ";
#      for my $col (sort { $a <=>  $b}  keys %{$matrix_s{$row} } ) {
#          print "$col  ";
#          my @a=@{$matrix_s{$row}{$col}};
#          foreach my $a (@a){ print $a};
#      }
#      print "}\n";
#  }
...
my $found = 0;
 ...
 foreach my $row ( sort {$a <=>  $b} keys %matrix_g ) {
#       print "$row: { ";
     foreach my $col ( sort {$a <=>  $b} keys %{$matrix_g{$row} } ) {
 #print "$col  ";
        my @g=@{$matrix_g{$row}{$col}};
  #      if (defined @{$matrix_s{$row}{$col}}) {
	my @s=@{$matrix_s{$row}{$col}};

#};

        if ($#g != $#s) {

            $cell_incorrect{$row}++; #print $cell_incorrect{$row};
	    #print "/INC/";
	    
        }
	elsif (@g == () and  @s == ()){
            
             $cell_correct_neg{$row}++;#print $cell_correct_neg{$row};
             #print "/--/";
        }
        elsif (same_cell(\@g, \@s)){

             $cell_correct_pos{$row}++;#print $cell_correct_pos{$row};
	  #  print "/COR/";
        }
        else {
...
foreach my $row (sort keys %cell_incorrect){

 # print "row $row cell incorrect ";print $cell_incorrect{$row};print "\n";
}

foreach my $row (sort keys %cell_correct_neg){

 # print "row $row cell cor neg ";print $cell_correct_neg{$row};print "\n";
}

foreach my $row (sort keys %cell_correct_pos){

  #   if ($cell_correct_pos{$row} != 0)   {
 # print "row $row cell corr pos ";print $cell_correct_pos{$row};print "\n";
#}
}

my $row_incorrect=0;
my $row_correct_pos=0;
my $row_correct_neg=0;

my $cell_incorrect_tot=0;
my $cell_correct_pos_tot=0;
my $cell_correct_neg_tot=0;


# row-wise calculation
    foreach my $row (sort keys %cell_incorrect)  { if ($cell_incorrect{$row} != 0)       {$row_incorrect++; #print "row incorrect $row_incorrect\n"; 
										      }};#print "row incorrect $row_incorrect\n";

# cell-wise calculation
   foreach my $row (sort keys %cell_incorrect)  { if ($row != 0) {$cell_incorrect_tot   +=   $cell_incorrect{$row} };   #print "cell incorrect $cell_incorrect_tot\n";
};
   foreach my $row (sort keys %cell_correct_pos){ if ($row != 0) {$cell_correct_pos_tot +=   $cell_correct_pos{$row} }; #print "cell correct $cell_correct_pos_tot\n";
};
   foreach my $row (sort keys %cell_correct_neg){ if ($row != 0 ){$cell_correct_neg_tot +=   $cell_correct_neg{$row} }; #print "cell correct neg $cell_correct_neg_tot\n";
};



 return ($row_incorrect, $cell_incorrect_tot, $cell_correct_pos_tot, $cell_correct_neg_tot);
...
# Verifies if two cells are equal,
#   i.e., their full argument sets must be identical
#
sub same_cell
{
    my ($matrix_g_cell, $matrix_s_cell) = @_;

    #foreach my $gold_arg (@matrix_g_cell) { print $gold_arg;}foreach my $sys_arg (@matrix_s_cell){print $sys_arg;}
    my $found = 0;

    
    # each gold arg must be matched in sys
    foreach my $gold_arg (@{$matrix_g_cell}) {
	$found = 0;
	foreach my $sys_arg (@{$matrix_s_cell}){

            

	    if ($gold_arg eq  $sys_arg){
		$found = 1;
		last if $found == 1;
	    }
	}
